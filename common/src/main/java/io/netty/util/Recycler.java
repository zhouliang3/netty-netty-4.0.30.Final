...
public abstract class Recycler<T> {
    ...
    static final class Stack<T> {
        ...
        DefaultHandle pop() {
            int size = this.size;
            if (size == 0) {
                if (!scavenge()) {//inote 如果elements数组中已经没有对象可用，则需要从仓库中查找是够有可以用的对象，也就是scavenge的实现，scavenge具体调用的是scavengeSome。
                    return null;
                }
                size = this.size;
            }
        ...
        boolean scavengeSome() {
            /**
             * inote Stack的仓库是由WeakOrderQueue连接起来的链表实现的，Stack维护着链表的头部指针。而每个WeakOrderQueue又维护着一个链表，
             * 节点由Link实现，Link的实现很简单，主要是继承AtomicInteger类，另外还有一个Handle数组、一个读指针和一个指向下一个节点的指针
             * ，Link巧妙的利用AtomicInteger值来充当数组的写指针从而避免并发问题。
             */
