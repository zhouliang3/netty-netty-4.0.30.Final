...
public abstract class SingleThreadEventExecutor extends AbstractScheduledEventExecutor {
    ...
    protected SingleThreadEventExecutor(
        ...
        taskQueue = newTaskQueue();//inote 任务队列
    }
    ...
    protected boolean runAllTasks(long timeoutNanos) {
        /**
         * inote fetchFromDelayedQueue()方法，由父类SingleThreadEventExecutor实现,其功能是将延迟任务队列（delayedTaskQueue）
         * 中已经超过延迟执行时间的任务迁移到非IO任务队列（taskQueue）中.然后依次从taskQueue取出任务执行，
         * 每执行64个任务，就进行耗时检查，如果已执行时间超过预先设定的执行时间，则停止执行非IO任务，避免非IO任务太多，影响IO任务的执行
         */
        fetchFromScheduledTaskQueue();
        Runnable task = pollTask();
        if (task == null) {
        ...
        for (;;) {
            ...
            if ((runTasks & 0x3F) == 0) {//inote 位运算判断是否为64，因runTasks是递增的所以可行
                lastExecutionTime = ScheduledFutureTask.nanoTime();
