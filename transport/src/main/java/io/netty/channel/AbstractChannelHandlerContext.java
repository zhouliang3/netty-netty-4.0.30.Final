...
abstract class AbstractChannelHandlerContext extends DefaultAttributeMap implements ChannelHandlerContext {
    ...
    public ChannelHandlerContext fireChannelRegistered() {
        final AbstractChannelHandlerContext next = findContextInbound();//idoubt 这个方法看不懂
        EventExecutor executor = next.executor();
        if (executor.inEventLoop()) {//inote true
    ...
    private void invokeChannelRegistered() {
        try {
            ((ChannelInboundHandler) handler()).channelRegistered(this);//inote ChannelInitializer.channelRegistered is invoked
        } catch (Throwable t) {
    ...
    public ChannelFuture bind(final SocketAddress localAddress, final ChannelPromise promise) {
        ...
        //inote 触发pipeline中的bind事件，最终起作用的是io.netty.channel.DefaultChannelPipeline.HeadContext.bind()，调用unsafe中的bind方法
        ...
        if (executor.inEventLoop()) {
            ...
            }, promise, null);//idoubt 这段代码是在何时调用的呢
        }
    ...
    private void invokeConnect(SocketAddress remoteAddress, SocketAddress localAddress, ChannelPromise promise) {
        try {
            ((ChannelOutboundHandler) handler()).connect(this, remoteAddress, localAddress, promise);//inote 只有一个outbound handler被执行DefaultChannelPipeline.HeadContext.connect()
        } catch (Throwable t) {
            notifyOutboundHandlerException(t, promise);
        }
    }
    ...
    private void invokeWrite(Object msg, ChannelPromise promise) {
        try {
            ((ChannelOutboundHandler) handler()).write(this, msg, promise);//inote DefaultChannelPipeline.HeadContext.write()
        } catch (Throwable t) {
            notifyOutboundHandlerException(t, promise);
        }
    }
    ...
    private void invokeFlush() {
        try {
            ((ChannelOutboundHandler) handler()).flush(this);//inote 由于flush是Outbound事件，所以会调用headHandler的flush方法
        } catch (Throwable t) {
            notifyHandlerException(t);
        }
    }
    ...
    private void write(Object msg, boolean flush, ChannelPromise promise) {
        ...
        EventExecutor executor = next.executor();
        //inote 在IO线程中执行， Write是一个Outbound事件，所以会调用outbound处理器的write方法。主要是Head处理器中的write方法调用unsafe的write方法
        if (executor.inEventLoop()) {
    ...
    private AbstractChannelHandlerContext findContextInbound() {//从Handler中找到inbound
        AbstractChannelHandlerContext ctx = this;
        do {
            ctx = ctx.next;//idoubt 看不懂这段是干嘛的
        } while (!ctx.inbound);
        return ctx;
    }

    private AbstractChannelHandlerContext findContextOutbound() {//inote 传入的是Tail即，从最后一个Handler向前查找所有的outBound，并依次触发bind事件
        AbstractChannelHandlerContext ctx = this;
        do {
            ctx = ctx.prev;
        } while (!ctx.outbound);
